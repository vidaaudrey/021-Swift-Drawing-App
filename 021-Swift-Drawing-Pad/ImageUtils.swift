...
extension CALayer {
    ...
    public func getPixelColorsAtPoint(centerPoint: CGPoint, dx: CGFloat, dy: CGFloat) -> [UIColor] {
        ...
        let ctx = CGBitmapContextCreate(&pixel, Int(dx) * 2 + 1, Int(dy) * 2 + 1, 8, 4 *  Int(columns), colorSpace, bitmapInfo)  // not sure why use 4 * columns, but it seems only work in this way
