...
public extension CGRect {
    ...
    // return the index value of a small rect within a bigger rect.It should return a tuple with possible nil value. Use CGPoint is just for convenience
    public func innerSquarePositionAsIndex(#innerRect: CGRect) -> CGPoint?{
        if self.contains(innerRect){
            let indexX = floor((innerRect.origin.x - self.origin.x) / innerRect.width)
            let indexY = floor((innerRect.origin.y - self.origin.y) / innerRect.height)
    ...
    public func innerSquarePositionAsIndex(#innerRectSize: CGSize, point: CGPoint) -> CGPoint?{
        let innerRectOriginXIndex = floor((point.x - self.origin.x) / innerRectSize.width)
        let innerRectOriginYIndex = floor((point.y - self.origin.y) / innerRectSize.height)
        let innerRect = CGRectMake(innerRectOriginXIndex * innerRectSize.width, innerRectOriginYIndex * innerRectSize.height, innerRectSize.width, innerRectSize.height)
        return self.innerSquarePositionAsIndex(innerRect: innerRect)
    }
...
//    }
   ...
   public func paddingBottom(targetHeight: CGFloat) -> CGRect{
    ...
    public func insetMaxSquare() -> CGRect {
        var width = self.width < self.height ? self.width : self.height
        return CGRect(x: self.midX - self.width / 2, y: self.midY - self.width / 2, width: width, height: width)
    }
    
    public func insetSquare(#dx: CGFloat) -> CGRect {
        if self.width < self.height {
            var width = self.width
            return CGRect(x: self.midX - self.width / 2 + dx, y: self.midY - self.width / 2 + dx, width: width - 2 * dx, height: width - 2 * dx)
        } else {
            return CGRect(x: self.midX - self.height / 2 + dx, y: self.midY - self.height / 2 + dx, width: height - 2 * dx, height: height - 2 * dx)
        }
    }
    ...
    public func maxRectWithinRectWithSize(sourceSize: CGSize) -> CGRect {
    ...
    public func maxRectWithinRectWithSizeAndPaddingRatio (sourceSize: CGSize, paddingRatio: CGFloat) -> CGRect {
        let tempRect = maxRectWithinRectWithSize(sourceSize)
   ...
   public  func maxRectWithinRectWithSizeAndPadding(sourceSize: CGSize, padding: CGFloat, paddingVertical: CGFloat = 0) -> CGRect {
        let tempRect = maxRectWithinRectWithSize(sourceSize)
        let offSetX = (width - tempRect.width)/2
        let offSetY = (height - tempRect.height)/2
